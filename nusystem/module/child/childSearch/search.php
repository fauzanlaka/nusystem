...
<table class="table">
  <tbody>
    <?php
        $q = $_POST['q'];
        $search = mysqli_query($con, "SELECT c.*,ct.*,cp.* FROM childs c
                              INNER JOIN childType ct ON c.ct_id=ct.ct_id
                              INNER JOIN childProject cp ON c.cp_id=cp.cp_id
                              WHERE c.c_fname LIKE '%".$q."%' OR c.c_fatherName LIKE '%".$q."%' OR c.c_motherName LIKE '%".$q."%' OR c.c_pFname LIKE '%".$q."%'
                              ORDER BY c.c_id DESC");
        while($rowChilds = mysqli_fetch_array($search)){
            $c_id = $rowChilds['c_id'];
    ?>
    <tr>
      <td width='10%'>
      ...
      <td align="right">
          <?php
            ...
            if($ct_id == 1){
                $ctId = "edit_1";
                $url = "module/child/childAdd/print/orphanData.php?id=$c_id";
            }else{
                $ctId = "edit_2";
                $url = "module/child/childAdd/print/poorData.php?id=$c_id";
            }
            ?>
          <h6><b>พิมพ์ &nbsp  &nbsp; แก้ไข &nbsp  &nbsp; ลบ </b></h6>
          <h4><a href="<?= $url ?>" target="_blank"><span class="glyphicon glyphicon-print"></span></a> | <a href="?page=child&&cpage=<?= $ctId ?>&&id=<?= $rowChilds['c_id'] ?>"><span class="glyphicon glyphicon-edit"></span></a> | <a href="?page=child&&cpage=delete&&id=<?= $rowChilds['c_id'] ?>" onclick="return confirm('คุณเเน่ใจหรือไม่ว่าจะลบข้อมูลนี้ ?');"><span class="glyphicon glyphicon-trash"></span></a></h4>
      </td>
    </tr>
    <?php
        }
