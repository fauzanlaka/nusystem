...
function portfolio_init() {
	// register Portfolio
	$args = array(
		'label' => __('Portfolio', 'uds-textdomain'),
		'singular_label' => __('Portfolio Item', 'uds-textdomain'),
		'public' => true,
		'show_ui' => true,
	...
	register_taxonomy('portfolio_category', 'uds-portfolio', array('label'=>__('Tags', 'uds-textdomain'), 'hierarchical' => false));
}
...
function uds_portfolio_scripts(){
	if(is_portfolio()){
		...
		wp_register_script("mousewheel", $theme."/fancybox/jquery.mousewheel-3.0.4.pack.js", array('jquery'));
		wp_register_script("fancybox", $theme."/fancybox/jquery.fancybox-1.3.4.pack.js", array('jquery', 'mousewheel', 'scripts'));
		wp_register_script("quicksand", $theme."/portfolio/js/jquery.quicksand.min.js", array('jquery'));
		wp_enqueue_script("portfolio", $theme."/portfolio/js/portfolio.min.js", array('jquery', 'fancybox', 'quicksand'));
	}
}
...
function uds_portfolio_styles(){
	if(is_portfolio()){ 
		...
		wp_enqueue_style('fancybox', $theme.'/fancybox/jquery.fancybox-1.3.4.css', false, false, 'screen');
	}
}
...
$page_meta_box = array(
	...
	'title' => __('Portfolio Options (Applies only to pages used for Portfolio)', 'uds-textdomain'),
	'page' => 'page',
	'context' => 'normal',
	'priority' => 'high',
	'fields' => array(
		array(
			'name' => __('Show only posts with this tag: ', 'uds-textdomain'),
			'desc' => __('Will not display tag selection', 'uds-textdomain'),
...
$meta_box = array(
	...
	'title' => __('Options', 'uds-textdomain'),
	'page' => 'uds-portfolio',
	'context' => 'normal',
	'priority' => 'high',
	'fields' => array(
		array(
			'name' => __('Content Type', 'uds-textdomain'),
			'id' => 'uds-portfolio-content-type',
			'type' => 'radio',
			'std' => '',
			'options' => array(
				array('name' => __('Image or other', 'uds-textdomain'), 'value' => 'image'),
				array('name' => __('Video', 'uds-textdomain'), 'value' => 'video')
			)
		),
		array(
			'name' => __('Do not open in lightbox', 'uds-textdomain'),
...
function uds_save_data($post_id) {
	...
	// verify nonce
	if (empty($_POST['uds_meta_box_nonce']) || !wp_verify_nonce($_POST['uds_meta_box_nonce'], basename(__FILE__))) {
		return $post_id;
	}
	...
	foreach ($box as $field) {
		...
		$new = isset($_POST[$field['id']]) ? $_POST[$field['id']] : $old;
