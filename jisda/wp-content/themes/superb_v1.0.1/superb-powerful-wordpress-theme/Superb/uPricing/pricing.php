...
define('UDS_PRICING_USE_COMPRESSION', true);
...
$uds_pricing_general_options = array(
	'name' => array(
		'type' => 'text',
		'label' => __('Name', 'uds-textdomain'),
		'default' => '',
		'tooltip' => ''
	),
	'skin' => array(
		'type' => 'select',
		'label' => __('Skin', 'uds-textdomain'),
		'options' => array(
			'default' => __('Default', 'uds-textdomain'),
			'dark' => __('Dark', 'uds-textdomain'),
			'light' => __('Light', 'uds-textdomain'),
			'blue-yellow' => __('Blue and yellow', 'uds-textdomain'),
			'lime' => __('Lime', 'uds-textdomain')
		),
		'default' => 'default',
		'tooltip' =>''
	)
...
$uds_pricing_column_options = array(
	'uds-name' => array(
		'type' => 'text',
		'label' => __('Product Name', 'uds-textdomain'),
		'default' => '',
		'tooltip' => __('Name of the product you are comparing', 'uds-textdomain')
	),
	'uds-price' => array(
		'type' => 'text',
		'label' => __('Product Price', 'uds-textdomain'),
		'default' => '',
		'tooltip' => __('Price of the product, include currency, exclude units', 'uds-textdomain')
	),
	'uds-unit' => array(
		'type' => 'text',
		'label' => __('Unit', 'uds-textdomain'),
		'default' => '',
		'tooltip' => __('Unit of the price of the product (e.g. /month, per piece, per kilo)', 'uds-textdomain')
	),
	'uds-link' => array(
		'type' => 'text',
		'label' => __('Call to action Link', 'uds-textdomain'),
		'default' => '',
		'tooltip' => __('URL of the product page', 'uds-textdomain')
	),
	'uds-link-label' => array(
		'type' => 'text',
		'label' => __('Call to action Label', 'uds-textdomain'),
		'default' => '',
		'tooltip' => __('Call to action button text (e.g. Buy This!)', 'uds-textdomain')
	),
	'uds-featured' => array(
		'type' => 'radiocross',
		'label' => __('Featured', 'uds-textdomain'),
		'default' => '',
		'tooltip' => __('One product can be featured', 'uds-textdomain')
	)
);
...
function uds_pricing_is_active()
{
	if(function_exists('uds_active_shortcodes')) {
		...
		if( ! in_array('uds-pricing-table', $active_shortcodes) && ! in_array('uds-pricing-product', $active_shortcodes)) {
			return false;
		}
	}
...
{	
	...
	$pricing_page = add_menu_page(__("uPricing", 'uds-textdomain'), __("uPricing", 'uds-textdomain'), 'manage_options', 'uds_pricing_admin', 'uds_pricing_admin', $icon, $position);
	$pricing_structure_page = add_submenu_page('uds_pricing_admin', __("Structure", 'uds-textdomain'), __('Structure', 'uds-textdomain'), 'manage_options', 'uds_pricing_structure', 'uds_pricing_structure');
	$pricing_products_page = add_submenu_page('uds_pricing_admin', __("Products", 'uds-textdomain'), __('Products', 'uds-textdomain'), 'manage_options', 'uds_pricing_products', 'uds_pricing_products');
...
function uds_pricing_render_product_options($product = null, $properties = array())
{
	...
	if(!empty($properties)) {
		echo "<div class='properties-separator'><h3>" . __('Properties', 'uds-textdomain') . "</h3></div>";
...
//	delete_option(UDS_PRICING_OPTION);
	if(isset($_POST['uds_pricing_nonce']) && wp_verify_nonce($_POST['uds_pricing_nonce'], 'uds-pricing-nonce')) {
		...
		if(empty($name)) {
			return new WP_Error("uds_pricing_table_name_empty", __("Table name can not be empty", 'uds-textdomain'));
		}
		...
		if(in_array($name, array_keys($pricing_tables)) && $name != $name_orig) {
			return new WP_Error("uds_pricing_table_name_taken", __("There already is a table named &quot;".$name."&quot;", 'uds-textdomain'));
		}
...
function uds_pricing_process_products()
{
	...
	if(!empty($_POST['uds_pricing_products_nonce']) && wp_verify_nonce($_POST['uds_pricing_products_nonce'], 'uds-pricing-products-nonce')) {
		...
		if(empty($pricing_table)) {
			return new WP_Error("uds_pricing_table_nonexistent", __("This pricing table does not exist!", 'uds-textdomain'));
		}
 ...
 *	@param string $theme pricing table skin
 *	@param string $product_name (optional) product name, will render just the product, if provided
 *	
 *	@return string rendered table
 */
function get_uds_pricing_table($name = '', $theme = '', $product_name = '')
{
	global $uds_pricing_general_options;
	$pricing_tables = maybe_unserialize(get_option(UDS_PRICING_OPTION, array()));
	
	if(empty($pricing_tables)) {
		return "<p class='error'>" . __('There are no Pricing Tables defined.', 'uds-textdomain') . "</p>";
	}
	...
	// sanity checks
	if(empty($pricing_table)) {
		return "<p class='error'>" . sprintf(__('Pricing Table named %s does not exist.', 'uds-textdomain'), "&quot;".htmlentities($name)."&quot;") . "</p>";
	}
	
	if(empty($pricing_table['properties'])) {
		return "<p class='error'>" . sprintf(__('Pricing Table named %s does not have any Properties.', 'uds-textdomain'), "&quot;".htmlentities($name)."&quot;") . "</p>";
	}
	
	if(empty($pricing_table['products'])) {
		return "<p class='error'>" . sprintf(__('Pricing Table named %s does not have any Products.', 'uds-textdomain'), "&quot;".htmlentities($name)."&quot;") . "</p>";
	}
	...
	$no_featured = $pricing_table['no-featured'] || !$display_product;
	...
	$headers = $footers = $header_shadow = $footer_shadow = '';
	// create headers and footers
	if(! $display_product) { // Full table mode
		if($pricing_table['products'][0]['uds-featured'] === true && !$no_featured) { // Render featured
	...
	for($i = 0; $i < $product_count; $i++) {
		...
		// Skip the other headers if we're in the Product mode
		if($product['uds-name'] != $product_name && $display_product) continue;
		...
		if($display_product) {
			$featured_prev = $featured = $featured_next = '';
		}
	...
	for($i = 0; $i < $property_count; $i++) {	
		...
		$properties = array_keys($pricing_table['properties']);
		$property_name = $properties[$i];
		foreach($product_matrix[$i] as $key => $cell) {
			// skip values if we're in the Product mode
			$product = isset($pricing_table['products'][$key]) ? $pricing_table['products'][$key]['uds-name'] : '';
			if($product != $product_name && $display_product) continue;
			...
			$property_element = empty($product_name) ? '' : "<span class='property'>$property_name</span>";
			$products .= "<$td class='row-$i column-$key $oddeven $featured_prev $featured $featured_next'>$cell $property_element</$td>";
		}
		$products .= '</tr>';
	}
	...
	// Add Class if in Product Mode
	$product_class = '';
	if(!empty($product_name)) {
		$product_class = 'uds-single-product';
	}
	
	// assemble the final table
	$out = "
		<div class='uds-pricing-table $skin $product_class $ie6'>
			<table>
...
add_shortcode('uds-pricing-product', 'uds_pricing_product');
function uds_pricing_product($atts, $content = null)
{
	extract(shortcode_atts(array(
		'table' => '',
		'product' => '',
	...
	return get_uds_pricing_table($table, $theme, $product);
}
