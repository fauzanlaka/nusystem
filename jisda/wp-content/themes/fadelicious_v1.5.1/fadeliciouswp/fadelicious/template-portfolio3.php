...
<?php if ( in_category($pcatid) ) {?>
    ...
    <?php if ( $main_video ) : ?>
    <a rel="zoombox[portfolio]" href="<?php echo get_post_meta($post->ID, "main_video", $single = true); ?>" alt="<?php the_title(); ?>" class="play2"><?php the_post_thumbnail( 'fade_thumbnail_bigger' ); ?> </a>
    

    <?php elseif ( $main_music ) : ?>
    
    
    <a rel="zoombox[portfolio]" href="<?php echo get_post_meta($post->ID, "main_music", $single = true); ?>" alt="<?php the_title(); ?>" class="play2"><?php the_post_thumbnail( 'fade_thumbnail_bigger' ); ?> </a>
    

    <?php else : ?>
    
    <a rel="zoombox[portfolio]" href="<?php echo $image_url;?>" alt="<?php the_title(); ?>" class="zoom2"><?php the_post_thumbnail( 'fade_thumbnail_bigger' ); ?></a>
    <?php endif; ?>
<!--End conditional check for video --> 
      <!-- End Full Size Link and Description -->
      <h4><a href="<?php the_permalink(); ?>"><?php the_title_shorten(26,'...'); ?></a></h4>
