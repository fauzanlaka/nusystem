...
if (  $wp_query->have_posts()) : while (have_posts()) : the_post(); 
    ...
    if ($attachments) {
      foreach($attachments as $gallery_image )                                                                 
      {
        ...
        echo '<li><a alt="'.$gallery_image->post_title.'" rel="zoombox['.$post->ID.']" href="'.$attachment_img.'" class="zoom">';
        echo  '<img src="'.get_bloginfo('template_url').'/functions/img_resize/timthumb.php?src='.$attachment_img.'&amp;w=435&amp;h=300&amp;zc=1" alt=""/>';
        echo '</a></li>';
      }
    }
?>
