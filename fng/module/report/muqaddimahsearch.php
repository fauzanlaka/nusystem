...
<?php
    ...
    $sql = "SELECT COUNT(s.student_id),s.*,m.* FROM students s 
         ...
         WHERE s.class='$class_s' and s.ft_id='$faculty_s' and s.dp_id='$department_s' and m.m_academicyear='$year_s'";
    ...
    $sql = "SELECT s.*,m.* FROM students s 
            LEFT JOIN muqaddimah_pay m ON s.st_id=m.st_id 
            WHERE s.class='$class_s' and s.ft_id='$faculty_s' and s.dp_id='$department_s' and s.income_year='$year_s' and m.m_academicyear='$year_s' ORDER BY s.student_id $limit";
    $query = mysqli_query($con, $sql);
...
<div class="pull-left">
        ...
        Fakulti : <b><?= $fakulty_name ?></b> , Jurusan : <b><?= $department_name ?></b> , Tahun pengajian : <?= $year_s ?></b> , Kelas : <b><?= $cname ?></b>
</div>
