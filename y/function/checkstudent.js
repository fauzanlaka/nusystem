$(document).ready(function(){
            $('#username').keyup(function(){ // Keyup function for check the user action in input
                ...
                if(Username.length > 2) { // check if greater than 2 (minimum 3)
                    ...
                    type : 'POST',
                    data : UrlToPass,
                    url  : 'function/student.php',
                    success: function(responseText){ // Get the result and asign to each cases
                        if(responseText == 0){
                            UsernameAvailResult.html('<span class="success">NO.POKOK Bisa di guna</span>');
                        }
                        else if(responseText > 0){
                            UsernameAvailResult.html('<span class="error">NO.POKOK Sudah ada</span>');
                        }
                        else{
                    ...
                    UsernameAvailResult.html('<span class="error">Sempurnakan NO.POKOK</span>');
                }
